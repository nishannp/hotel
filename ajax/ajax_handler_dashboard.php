<?php
// ajax/ajax_handler_dashboard.php - Updated for Party System
...
try {
    ...
    // =================================================================
    // 1. KPI Stats & Comparisons (No changes needed here)
    // =================================================================
    $data['kpi'] = [];
    // Hotel Revenue
    ...
    // Avg. Customer Time (Today) - Note: This is an approximation based on order and payment time.
    ...
    // =================================================================
    // 3. Sales Chart Data (Last 30 Days) (No changes needed)
    // =================================================================
    ...
    while($row = $store_sales_res->fetch_assoc()){
        $daily_sales[$row['sale_date']] = ($daily_sales[$row['sale_date']] ?? 0) + (float)$row['daily_total'];
    }
    for ($i = 29; $i >= 0; $i--) {
    ...
    // =================================================================
    // 4. Menu & Inventory Insights (No changes needed)
    // =================================================================
    $data['menu_insights'] = [];
    ...
    // =================================================================
    // 5. Active Orders - UPDATED
    // =================================================================
    $active_orders_res = $conn->query("
        SELECT o.OrderID, rt.TableNumber, cp.PartyIdentifier, o.TotalAmount, o.OrderStatus 
        FROM orders o
        ...
        ORDER BY o.OrderTime ASC LIMIT 10
    ");
    $data['active_orders'] = $active_orders_res->fetch_all(MYSQLI_ASSOC);

    // =================================================================
    // 6. Advanced Analytics (No changes needed)
    // =================================================================
    $data['analytics'] = [];
    ...
    // All order *details* for today for the drill-down modal (No changes needed)
    ...
    // All other reports (No changes needed)
    $report_menu_sales_res = $conn->query("SELECT mi.Name as ItemName, mc.CategoryName, SUM(od.Quantity) as QuantitySold, SUM(od.Subtotal) as TotalRevenue FROM order_details od JOIN orders o ON od.OrderID = o.OrderID JOIN menu_items mi ON od.MenuItemID = mi.MenuItemID JOIN menu_categories mc ON mi.CategoryID = mc.CategoryID WHERE o.OrderTime BETWEEN '$today_start' AND '$today_end' AND o.OrderStatus = 'Completed' GROUP BY mi.MenuItemID, mc.CategoryName ORDER BY TotalRevenue DESC");
    $data['todays_report']['menu_sales'] = $report_menu_sales_res->fetch_all(MYSQLI_ASSOC);
    $payment_methods_res = $conn->query("SELECT p.PaymentMethod, SUM(p.AmountPaid) as TotalAmount FROM payments p JOIN orders o ON p.OrderID = o.OrderID WHERE p.PaymentTime BETWEEN '$today_start' AND '$today_end' GROUP BY p.PaymentMethod");
    $data['todays_report']['payment_methods'] = $payment_methods_res->fetch_all(MYSQLI_ASSOC);
    $staff_performance_res = $conn->query("SELECT s.FirstName, s.LastName, COUNT(o.OrderID) as OrderCount, SUM(CASE WHEN o.OrderStatus = 'Completed' THEN o.TotalAmount ELSE 0 END) as TotalRevenue FROM orders o JOIN staff s ON o.StaffID = s.StaffID WHERE o.OrderTime BETWEEN '$today_start' AND '$today_end' GROUP BY s.StaffID ORDER BY TotalRevenue DESC");
    $data['todays_report']['staff_performance'] = $staff_performance_res->fetch_all(MYSQLI_ASSOC);
    $report_store_sales_res = $conn->query("SELECT s.SaleTime, sic.CategoryName, si.Name as ItemName, s.Quantity, s.SalePrice, s.TotalAmount FROM store_sales_log s JOIN store_items si ON s.StoreItemID = si.StoreItemID JOIN store_item_categories sic ON si.CategoryID = sic.CategoryID WHERE s.SaleTime BETWEEN '$today_start' AND '$today_end' ORDER BY s.SaleTime DESC");
